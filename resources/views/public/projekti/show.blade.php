    ...
    <article class="content-card reveal-up">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                ...
                <h1 class="mt-2 text-3xl font-semibold tracking-tight">{{ $projekts->nosaukums }}</h1>
                ...
                <div class="mt-3 flex flex-wrap gap-2 text-sm text-zinc-600">
                    ...
                    <span>Sākums: {{ optional($projekts->sakuma_datums)->format('d.m.Y') }}</span>
                    @if($projekts->beigu_datums)
                        <span>· Beigas: {{ optional($projekts->beigu_datums)->format('d.m.Y') }}</span>
                    @endif
                </div>
            </div>

            <a class="btn-secondary btn-pill" href="{{ route('public.projekti.index') }}">← Atpakaļ</a>
        </div>
